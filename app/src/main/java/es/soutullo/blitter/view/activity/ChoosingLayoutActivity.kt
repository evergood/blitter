...
abstract class ChoosingLayoutActivity : AppCompatActivity(), IChoosableItemsListHandler {
    ...
    override fun onChosenItemsChanged() {
        ...
        val checkAll = (this.itemsAdapter.items.size == this.itemsAdapter.getSelectedIndexes().size)

        this.findViewById<TextView>(R.id.selected_items_count_text).text =
            this.itemsAdapter.getSelectedIndexes().size.toString()
    ...
    private fun changeBarLayout(choosingMode: Boolean) {
        ...
        this.findViewById<ViewGroup>(R.id.action_bar_content).visibility =
            if (choosingMode) View.VISIBLE else View.GONE
