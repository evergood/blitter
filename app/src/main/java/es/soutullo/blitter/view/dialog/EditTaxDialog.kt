...
class EditTaxDialog(context: Context, handler: IDialogHandler, private val currentTax: Double) :
    PromptDialog(
        context,
        handler,
        context.getString(R.string.dialog_edit_tax_title),
        context.getString(R.string.generic_dialog_cancel),
        context.getString(R.string.generic_dialog_positive_button),
        ""
    ) {

    @SuppressLint("InflateParams")
    override fun getCustomView(): View {
        val binding = DataBindingUtil.inflate<DialogEditTaxBinding>(
            LayoutInflater.from(this.context),
            R.layout.dialog_edit_tax,
            null,
            false
        )
