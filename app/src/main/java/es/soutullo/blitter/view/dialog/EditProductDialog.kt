...
class EditProductDialog(
    ...
    title: String,
    private val product: ManualTranscriptionProduct
    ...
    fun getNewProduct(): ManualTranscriptionProduct? {
        val name = this.dialog.findViewById<EditText>(R.id.dialog_edit_product_name)?.text?.trim()
            ?.toString()
        val price =
            this.dialog.findViewById<EditText>(R.id.dialog_edit_product_price)?.text?.toString()
                ?.toDoubleOrNull()
        val quantity =
            this.dialog.findViewById<EditText>(R.id.dialog_edit_product_quantity)?.text?.toString()
                ?.toIntOrNull()
    ...
    override fun getCustomView(): View {
        val binding = DataBindingUtil.inflate<DialogEditProductBinding>(
        ...
        with(binding.root) {
            ...
            findViewById<EditText>(R.id.dialog_edit_product_price).setText(
                BlitterUtils.getEditablePriceAsString(
                    product.unitPrice
                )
            )
    ...
    override fun onDialogCreated() {
        this.dialog.findViewById<EditText>(R.id.dialog_edit_product_quantity)?.filters =
            arrayOf(InputFilterMinMax(1, 50))
    }

    override fun preventDismissOnButtonClicked(): Boolean = true
    override fun getPositiveText(): String =
        this.context.getString(R.string.edit_product_dialog_positive_button)
