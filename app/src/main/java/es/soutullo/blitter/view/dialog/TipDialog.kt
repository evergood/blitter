...
class TipDialog(context: Context, handler: IDialogHandler?, private val bill: Bill) :
    ...
    override fun getCustomView(): View {
        this.binding = DataBindingUtil.inflate<DialogTipBinding>(
    ...
    private fun getDefaultTipPercent(): Double {
        ...
        val defaultPreferenceValue =
            this.context.resources.getInteger(R.integer.default_tip_percent)

        return PreferenceManager.getDefaultSharedPreferences(this.context)
            .getInt(preferenceKey, defaultPreferenceValue) / 100.0
    }

    private fun getStartingTipPercent() =
        this.bill.tipPercent.let { if (it < 0) this.getDefaultTipPercent() else it }

    override fun getPositiveText(): String =
        this.context.getString(R.string.generic_dialog_continue)
