...
class AssignationActivity : ChoosingLayoutActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this.findViewById<View>(R.id.assignation_root)
            .post { this.itemsAdapter.notifyDataSetChanged() }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_assign -> {
                val selectedLines = this.itemsAdapter.items.filterIndexed { index, _ ->
    ...
    private fun onTipPercentageConfirmed(tipPercent: Double) {
        val skipAds = PreferenceManager.getDefaultSharedPreferences(this)
            .getBoolean(this.getString(R.string.preference_key_ads_removed), false)
    ...
    private fun onAssignClicked(billLinesToAssign: List<BillLine>) {
        this.assignationDialog = AssignationDialog(
            this, this.createAssignationDialogHandler(),
    ...
    fun onAddNewPersonClicked() {
        PromptDialog(
            this,
            this.createNewPersonDialogHandler(),
            this.getString(R.string.dialog_add_person_title),
            this.getString(R.string.generic_dialog_cancel),
            this.getString(R.string.dialog_add_person_positive_button),
            this.getString(R.string.dialog_add_person_field_hint)
        ).show()
    }
    ...
    private fun onClearAssignationsClicked() {
        ...
        ConfirmationDialog(
            this, this.createClearAssignationsDialogHandler(selectedLines), dialogTitle,
            dialogMessage, positiveButtonText, negativeButtonText
        ).show()
    }
    ...
    private fun onNewPersonAdded(newPersonName: String) {
        ...
        this.assignationDialog?.updatePeopleList(
            this.peopleAddedOnSession + DaoFactory.getFactory(this)
                .getPersonDao().queryRecentPersons(10, this.peopleAddedOnSession)
        )
    }
    ...
    fun onDeleteRecentPersonClicked(name: String) {
        ...
        ConfirmationDialog(
            this, this.createDeleteRecentPersonDialogHandler(name), dialogTitle, dialogMessage,
     ...
     */
    private fun onAssignationDone(
        affectedBillLines: List<BillLine>,
        assignedPersons: List<Person>,
        unassignedPersons: List<Person>
    ) {
        ...
        assignedPersons.forEach {
            DaoFactory.getFactory(this).getPersonDao().insertRecentPerson(it)
        }
    ...
    private fun onTryToFinishWithMissingAssignation() {
        Toast.makeText(
            this,
            this.getString(R.string.toast_assignation_must_assign_all),
            Toast.LENGTH_SHORT
        ).show()
    }
    ...
    private fun prepareIdResult() {
        this.setResult(
            Activity.RESULT_OK,
            Intent().putExtra(BillSummaryActivity.INTENT_DATA_RETURNED_BILL_ID, this.bill.id)
        )
    }
        ...
        return object : IDialogHandler {
            override fun onPositiveButtonClicked(dialog: CustomDialog) {
                ...
                if (dialog is AssignationDialog) {
                    onAssignationDone(
                        dialog.billLines,
                        dialog.getAssignedPersons(),
                        dialog.getUnassignedPersons()
                    )
                }
            }
    ...
    private fun createClearAssignationsDialogHandler(affectedBillLines: List<BillLine>): IDialogHandler {
        return object : IDialogHandler {
            override fun onPositiveButtonClicked(dialog: CustomDialog) {
                this@AssignationActivity.onAssignationDone(
                    affectedBillLines,
                    listOf(),
                    affectedBillLines.map { it.persons }.flatten()
                )
            }
    ...
    private fun createTipDialogHandler(): IDialogHandler {
        return object : IDialogHandler {
            override fun onPositiveButtonClicked(dialog: CustomDialog) {
                (dialog as? TipDialog)?.getTipPercent()
                    ?.let { this@AssignationActivity.onTipPercentageConfirmed(it) }
            }
